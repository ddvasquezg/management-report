...
@Component({
  selector: 'app-leaders-filter',
  standalone: true,
  imports: [FormsModule],
  template: `
    <div class="panel panel-narrow">
      <div class="panel-header"><h3>Lideres</h3></div>
      @if (selectedCount() > 0) {
      ...
      <div class="search-wrap">
        ...
        />
        @if (query) {
          <button class="search-clear" (click)="query = ''" aria-label="Limpiar busqueda">
...
export class LeadersFilterComponent {
  ...
  isSelected(name: string): boolean {
    return this.store.isLeaderSelected(name);
  }
}
